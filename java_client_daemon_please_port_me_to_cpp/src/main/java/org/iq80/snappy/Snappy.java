...
public final class Snappy
{
    ...
    /**
     * Uses the stream marker bytes to determine if the {@link org.iq80.snappy.SnappyFramedInputStream} or
     * {@link org.iq80.snappy.SnappyInputStream} should be used to decompress the content of <i>source</i>.
     *
     * @param source The compressed content to decompress. Must {@link java.io.InputStream#markSupported()
     * support} {@link java.io.InputStream#mark(int).}
     * @param verifyChecksums Indicates if the crc32-c checksums should be calculated and verified.
     * @return An appropriate {@link java.io.InputStream} implementation to decompress the content.
     * @throws IllegalArgumentException If <i>source</i> does not {@link java.io.InputStream#markSupported()
            ...
            throws IOException
    {
        ...
        if (buffer[0] == SnappyFramed.HEADER_BYTES[0]) {
            checkArgument(Arrays.equals(Arrays.copyOf(buffer, SnappyFramed.HEADER_BYTES.length), SnappyFramed.HEADER_BYTES), "invalid header");
            return new SnappyFramedInputStream(source, verifyChecksums);
        }
        else {
            checkArgument(Arrays.equals(Arrays.copyOf(buffer, SnappyOutputStream.STREAM_HEADER.length), SnappyOutputStream.STREAM_HEADER), "invalid header");
